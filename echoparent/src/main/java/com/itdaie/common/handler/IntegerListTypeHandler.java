...
public class IntegerListTypeHandler extends BaseTypeHandler<List<Integer>> {
    ...
    private List<Integer> toList(Array sqlArray) throws SQLException {
        ...
        // PostgreSQL integer[] 返回的可能是 Integer[] / int[] / Long[] / long[] / Number[]
        if (arrayObj instanceof Integer[]) {
            ...
            return Arrays.stream((Long[]) arrayObj).map(Long::intValue).collect(Collectors.toList());
        } else if (arrayObj instanceof long[]) {
            long[] longArray = (long[]) arrayObj;
            return Arrays.stream(longArray).boxed().map(Long::intValue).collect(Collectors.toList());
        } else if (arrayObj instanceof Number[]) {
            return Arrays.stream((Number[]) arrayObj).map(Number::intValue).collect(Collectors.toList());
        }
        return Collections.emptyList();
    }
}
